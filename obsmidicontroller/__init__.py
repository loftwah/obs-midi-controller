...
class OBSMidi:
  ...
  def print_message(self, message):
    if message.type == "note_off":
      ...
      if int(message.channel) == self.config['controller']['channel']['id'].get():
        ...
        print(reqdefs)
        print(message.note in reqdefs)
        if message.note in reqdefs:
            self.client.call(obswebsocket.requests.StartStopStreaming())
        streamdefs=self.config['controller']['notes']['streaming'].get()
        print(streamdefs)
        if message.note in streamdefs:
            self.client.call(obswebsocket.requests.StartStopRecording())
        transdefs=self.config['controller']['notes']['transitions'].get()
        if message.note in transdefs:
            status=self.client.call(obswebsocket.requests.GetStudioModeStatus())
            trans=self.client.call(obswebsocket.requests.GetTransitionList()).getTransitions()
            print(trans)
            if transdefs.index(message.note)<len(trans):
                name=trans[transdefs.index(message.note)]['name']
                print(u"Transition {}".format(name))
                self.client.call(obswebsocket.requests.SetCurrentTransition(name))
                if status.getStudioMode():
                    self.client.call(obswebsocket.requests.TransitionToProgram(name))
        ...
        if message.note in mutedefs:
            ...
            sources = self.client.call(obswebsocket.requests.GetCurrentScene()).getSources()
            audiosources=[]
            try:
            ...
            if mutedefs.index(message.note)<len(audiosources):
                ...
                self.client.call(obswebsocket.requests.ToggleMute(name))
        ...
        if int(message.channel) == self.config['controller']['channel']['id'].get():
            ...
            if message.control in voldefs:
                ...
                if voldefs.index(message.control)<len(audiosources):
                    name=audiosources[voldefs.index(message.control)]
                    self.client.call(obswebsocket.requests.SetVolume(name,(float(message.value)/127.0)))
    else:
        print(message)
  ...
  def initUI(self):
      ...
      choices=self.config['obsserver'].keys()
      print(choices)
